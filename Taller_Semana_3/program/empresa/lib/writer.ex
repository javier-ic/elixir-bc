defmodule Writer do
  ...
  @doc """
  Delete existing employees from the JSON file.
  ...
  def delete_employee(
    ...
    employees = read_employees(filename)
    updated_employee = Enum.filter(employees, fn e -> e.id != id end)
    json_data = Jason.encode!(updated_employee, pretty: true)
    File.write(filename, json_data)
  end

  @doc """
  Update an existing employees from the JSON file.
  ...
  ## Examples
      iex> Writer.update_employee(3, employee1)
      :ok
  """
  @spec update_employee(integer(), Employee.t(), String.t()) :: :ok | {:error, term()}
  def update_employee(
        id,
        %Employee{} = employee,
        filename \\ "employees.json"
    ...
    employees = read_employees(filename)
    employee_id = Enum.find_index(employees, fn e -> e.id == id end)

    updated_employee =
      List.update_at(employees, employee_id, fn _ -> %Employee{employee | id: id} end)
